  ...
  const fetchUserData = async () => {
    try {
      if (currentUser.role_type === 'farm') {
        ...
        const employeeResponse = await axios.get(`https://walrus-app-bfv5e.ondigitalocean.app/farm-board-be2/api/v1/users/${currentUser.id}/employees`);
        ...
        try {
          const imageResponse = await axios.get(`https://walrus-app-bfv5e.ondigitalocean.app/farm-board-be2/api/v1/users/${currentUser.id}/employees/image`);
          console.log('drawer image data:', imageResponse.data);
          setAvatarImage(imageResponse.data.image_url);
          setUserAvatar(imageResponse.data.image_url);
        } catch (imageError) {
