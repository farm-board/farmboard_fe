...
export default function EmployeeProfileEditDetails() {
  ...
  const handleSubmit = () => {
    axios.put(`https://walrus-app-bfv5e.ondigitalocean.app/farm-board-be2/api/v1/users/${currentUser.id}/employees`, { employee: data})
    .then(response => {
  ...
  const fetchProfileData = async () => {
    try {
      const [employeeResponse, imageResponse] = await Promise.all([
        ...
        axios.get(`https://walrus-app-bfv5e.ondigitalocean.app/farm-board-be2/api/v1/users/${currentUser.id}/employees/image`).catch(error => {
